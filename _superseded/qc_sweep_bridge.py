#!/usr/bin/env python3
"""qc_sweep v2 (bridge) -- ASSEMBLY INTERFERENCE GATE for findings 97-99.
Manifest swaps in bridge-arm 51, fixture r5.4, star r4; arm 47 + cam removed.
Standing rule: any unwhitelisted VIOLATION is stop-and-tell."""
import struct, numpy as np
PROG=(-36.75,0.0); DRIVE=(36.75,0.0)
FLUSH=0.12; ZBAND=0.5
def load(fn,zoff=0.0,flip=False,dx=0.0,dy=0.0,crop=None):
    ...
    f.close()
    if crop: lo,hi=crop; m=(V[:,0]>=lo)&(V[:,0]<=hi); V=V[m]
    V[:,0]+=dx; V[:,1]+=dy
    V[:,2]=(zoff-V[:,2]) if flip else (V[:,2]+zoff)
    return V
MANIFEST=[
 ("fixture","49_fixture_r54_v16.stl",0.0,False,0,0,None,None,"frame"),
 ("bridge","51_bridge_arm_v16.stl",2.5,False,PROG[0],PROG[1],None,None,"frame"),
 ("star","50_detent_star_r4_v16.stl",5.0,True,PROG[0],PROG[1],None,PROG,"board"),
...
WHITELIST={("star","bridge"),("board","wheel"),("board","fixture"),
           ("star","fixture"),("wheel","fixture"),("star","wheel"),("bridge","fixture")}
parts={}
for name,fn,zoff,flip,dx,dy,crop,axis,grp in MANIFEST:
    V=load(fn,zoff,flip,dx,dy,crop)
    if V is None or len(V)==0: print(f"  [skip] {name} ({fn})"); continue
    parts[name]=(V,axis,grp)
print("=== QC SWEEP v2 (BRIDGE, findings 97-99) ===")
viol=0
        ...
        if m.sum():
            ...
            for on,(OV,oax,ogrp) in parts.items():
                ...
                if mm.sum():
                    ...
                    if not pair:
                        viol+=1; print(f"  {name} vs {on}: z{zz:.1f}-{zz+ZBAND:.1f}  *** VIOLATION ***")
        zz+=ZBAND
